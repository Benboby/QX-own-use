 ...
 *  更新地址:
 * https://raw.githubusercontent.com/Peng-YM/QuanX/master/Tasks/nCov.js
 */
...
!(async () => {
   ...
   const province =
       await $.http.get(`https://apis.map.qq.com/ws/location/v1/ip?key=${key}`)
           .then(resp => {
   ...
   const newslist =
       await $.http
           .get({
             url:
                 'http://api.tianapi.com/txapi/ncov/index?key=45084b278cdc80d2b70969af2f249fa8',
             headers,
           })
   ...
   let detail = '\n「全国数据」' +
       '\n\n    -新增确诊: ' + desc.confirmedIncr +
       '\n    -现有确诊: ' + desc.currentConfirmedCount +
       '\n    -累计确诊: ' + desc.confirmedCount +
       '\n    -治愈: ' + desc.curedCount + '\n    -死亡: ' + desc.deadCount +
       '\n\n「疫情动态」\n\n     ' + news.title + '\n\n「动态详情」\n\n     ' +
       news.summary + '\n\n    发布时间：' + news.pubDateStr;
...
function ENV() {
  ...
  return {
    isQX: e, isLoon: t, isSurge: s, isNode: 'function' == typeof require && !i,
        isJSBox: i, isRequest: 'undefined' != typeof $request,
                           ...
                           (e, t) => {
                               d = setTimeout(
                                   () =>
                                       (c.onTimeout(),
                                        t(`${u} URL: ${
                                            l.url} exceeds the timeout ${
                                            a} ms`)),
                                   a)}) :
                       null;
                   return (p ? Promise.race([p, f]).then(
                                   e => (clearTimeout(d), e)) :
                               f)
  ...
  return new class {
    ...
    initCache() {
      if (s && (this.cache = JSON.parse($prefs.valueForKey(this.name) || '{}')),
          (i || n) &&
              (this.cache =
                   JSON.parse($persistentStore.read(this.name) || '{}')),
          o) {
        let e = 'root.json';
        this.node.fs.existsSync(e) ||
            this.node.fs.writeFileSync(
                e, JSON.stringify({}), {flag: 'wx'}, e => console.log(e)),
            this.root = {}, e = `${this.name}.json`,
            this.node.fs.existsSync(e) ?
            this.cache =
                JSON.parse(this.node.fs.readFileSync(`${this.name}.json`)) :
            (this.node.fs.writeFileSync(
                 e, JSON.stringify({}), {flag: 'wx'}, e => console.log(e)),
             this.cache = {})
      }
    ...
    persistCache() {
      ...
      s && $prefs.setValueForKey(e, this.name),
          ...
          o &&
          (this.node.fs.writeFileSync(
               `${this.name}.json`, e, {flag: 'w'}, e => console.log(e)),
           this.node.fs.writeFileSync(
               'root.json', JSON.stringify(this.root, null, 2), {flag: 'w'},
               e => console.log(e)))
    }
    write(e, t) {
    ...
    read(e) {
      return this.log(`READ ${e}`),
             -1 === e.indexOf('#') ? this.cache[e] :
                                     (e = e.substr(1),
                                      n || i ? $persistentStore.read(e) :
                                               s ? $prefs.valueForKey(e) :
                                                   o ? this.root[e] : void 0)
    }
    delete(e) {
    ...
    notify(e, t = '', l = '', h = {}) {
      ...
      if (s && $notify(e, t, l, h),
          ...
          i) {
        let s = {};
        a && (s.openUrl = a), c && (s.mediaUrl = c),
            '{}' === JSON.stringify(s) ? $notification.post(e, t, l) :
    ...
    wait(e) {
      ...
      s || i || n ? $done(e) :
                    o && !r && 'undefined' != typeof $context &&
              ($context.headers = e.headers, $context.statusCode = e.statusCode,
               $context.body = e.body)
    }
    stringify(e) {
